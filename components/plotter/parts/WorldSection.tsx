...
export const WorldSection: React.FC<WorldSectionProps> = ({ 
  title, icon, items, renderItem, compact = false, 
  onConsult, onAdd, onEdit, onDelete 
}) => (
    <div className="space-y-4">
        <SectionHeader 
          ...
          title={title} 
          action={onAdd && (
            <button onClick={onAdd} className="p-1.5 bg-stone-800 hover:bg-orange-600 hover:text-white text-stone-500 rounded-lg transition-all" title="新規追加">
              <Plus size={14}/>
            </button>
          )}
        />

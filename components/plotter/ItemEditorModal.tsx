    ...
    if (error.message?.includes('SAFETY_BLOCK')) {
      uiDispatch(
        Actions.openDialog({
          ...
          message:
            '生成内容が安全ガイドラインに抵触したため、処理が中断されました。入力内容や設定を見直してください。',
        }),
      );
      ...
      <div className="bg-stone-900 w-full max-w-lg rounded-2xl border border-stone-800 shadow-2xl flex flex-col max-h-[90vh]">
        <div className="p-4 border-b border-white/5 flex justify-between items-center bg-stone-900/50 rounded-t-2xl shrink-0">
          <span className="text-sm font-black text-stone-200 uppercase tracking-widest">
            {initialData?.id ? '編集' : '新規追加'}: {ITEM_LABELS[type]}
          </span>
              ...
              <div className="grid grid-cols-1 gap-3 animate-fade-in">
                {brainstormResults.map((idea, idx) => (
                  <Card
                    ...
                    padding="sm"
                    className="cursor-pointer hover:border-orange-500/40 group border border-white/5"
                    onClick={() => applyIdea(idea)}
                  >
                    <div className="flex justify-between items-start mb-1">
                      <span className="text-[11px] font-bold text-stone-200">
                        {idea.name || idea.title || idea.event || idea.concept}
                      </span>
                      {idea.concept_note && (
                    ...
                    <p className="text-[10px] text-stone-400 font-serif leading-relaxed line-clamp-2">
                      {idea.description || idea.summary || idea.definition}
                    </p>
                    <div className="mt-2 text-center opacity-0 group-hover:opacity-100 transition-opacity">
                      <span className="text-[9px] text-orange-400 font-black uppercase tracking-widest flex items-center justify-center gap-1">
                        <PlusCircle size={10} /> 採用する
                      </span>
                    </div>
                  </Card>
                ))}

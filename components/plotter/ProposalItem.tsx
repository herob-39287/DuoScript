...
export const ProposalItem = React.memo(
  ({ op, bible, chapters, isExpanded, onToggle, onAccept, onReject }: ProposalItemProps) => {
    ...
    if (isForeshadowingAction) {
      ...
      else if ((op.value as any)?.clues?.length > 0 || (op.value as any)?.redHerrings?.length > 0)
        foreshadowingActionType = 'progress';
    }
    ...
    return (
      <div
        className={`glass-bright rounded-2xl border transition-all ${isExpanded ? (isHypothetical ? 'border-indigo-500/40 shadow-[0_10px_40px_-10px_rgba(99,102,241,0.3)]' : 'border-orange-500/40 shadow-2xl') : 'border-white/5'} ${isSemanticInvalid ? 'border-rose-500/40' : ''}`}
      >
        <button
          onClick={onToggle}
        ...
        >
          <div className="flex-1 min-w-0 pr-4">
            <div className="flex items-center gap-2 mb-1">
              <span
                className={`text-[7px] font-black px-1.5 py-0.5 rounded uppercase ${isHypothetical ? 'bg-indigo-500/20 text-indigo-400' : isSemanticInvalid ? 'bg-rose-500/10 text-rose-400' : 'bg-orange-400/10 text-orange-400'}`}
              >
              ...
              {isForeshadowingAction && (
                <>
                  {foreshadowingActionType === 'plant' && (
                    <span className="text-[7px] font-black bg-emerald-500/20 text-emerald-400 px-1.5 py-0.5 rounded uppercase">
                      Plant 🌱
                    </span>
                  )}
                  {foreshadowingActionType === 'payoff' && (
                    <span className="text-[7px] font-black bg-purple-500/20 text-purple-400 px-1.5 py-0.5 rounded uppercase">
                      Payoff 💥
                    </span>
                  )}
                  {foreshadowingActionType === 'progress' && (
                    <span className="text-[7px] font-black bg-blue-500/20 text-blue-400 px-1.5 py-0.5 rounded uppercase">
                      Clue 🔍
                    </span>
                  )}
                </>
            ...
            <div
              className={`text-[11px] font-bold truncate ${isHypothetical ? 'text-indigo-200' : isSemanticInvalid ? 'text-rose-200' : 'text-stone-100'}`}
            >
              {renderTargetName()}{' '}
              {op.field && <span className="text-stone-600 ml-1">({op.field})</span>}
            </div>
          </div>
          {isExpanded ? (
          ...
          <div className="px-4 pb-4 space-y-4 animate-fade-in border-t border-white/5 pt-4 overflow-hidden">
            {isHypothetical && (
              <div className="p-3 bg-indigo-500/5 border border-indigo-500/10 rounded-xl space-y-1">
                <div className={`${Styles.text.labelSm} text-indigo-400 flex items-center gap-1`}>
                  <Beaker size={10} /> {t('sync.nexus.title', lang)}
                </div>
                <Txt variant="bodySm" className="text-indigo-300/70 italic">
                  {t('sync.nexus.desc', lang)}
                </Txt>
              </div>
            )}
            ...
            {isSemanticInvalid && (
              <div className="space-y-3">
                <div className="p-3 bg-rose-500/10 border border-rose-500/20 rounded-xl space-y-1">
                  <div className={`${Styles.text.labelSm} text-rose-400 flex items-center gap-1`}>
                    <AlertCircle size={10} /> {t('sync.status.needs_resolution', lang)}
                  </div>
                  <Txt variant="bodySm" className="text-rose-200 italic">
                    {op.resolutionHint || t('sync.msg.unidentified', lang)}
                  </Txt>
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      {isSearching ? <X size={10} /> : <Search size={10} />}{' '}
                      {isSearching ? t('sync.action.close', lang) : t('sync.action.search', lang)}
                    </button>
                  </div>
                  ...
                  {isSearching ? (
                    <div className="space-y-2 animate-fade-in">
                      ...
                      <div className="max-h-32 overflow-y-auto no-scrollbar space-y-1">
                        {filteredTargets.map((t) => (
                          <button
                            key={t.id}
                            onClick={() => handleSelectCandidate(t.id, t.name)}
                            className="w-full text-left p-2 hover:bg-white/5 rounded-lg flex items-center justify-between group"
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[10px] text-stone-300 group-hover:text-white">
                                {t.name}
                              </span>
                            </div>
                            <Check
                        ...
                        <button
                          onClick={handleSetAsNew}
                          className="w-full text-left p-2 hover:bg-orange-600/10 rounded-lg flex items-center gap-2 group border border-dashed border-stone-800"
                        >
                          <Plus size={10} className="text-orange-400" />
                          <span className="text-[10px] text-orange-400">
                            {t('sync.action.add_new', lang)}
                          </span>
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-1">
                      {(op.candidates || []).map((cand) => (
                        <button
                          key={cand.id}
                          onClick={() => handleSelectCandidate(cand.id, cand.name)}
                          className="flex items-center justify-between p-3 bg-stone-900 border border-white/5 rounded-xl hover:border-orange-500/30 transition-all group"
                        >
                          <div className="flex flex-col">
                            <span className="text-[10px] font-bold text-stone-200 text-left">
                              {cand.name}
                            </span>
                            <span className={Styles.text.labelXxs}>
                              {t(cand.reason, lang) || cand.reason} (
                              {t('sync.label.confidence', lang)}:{' '}
                              {Math.round(cand.confidence * 100)}%)
                            </span>
                          </div>
                          <ArrowRight
                            size={12}
                            className="text-stone-700 group-hover:text-orange-400 group-hover:translate-x-1 transition-all"
                          />
                        </button>
                      ))}
                      <button
                        onClick={handleSetAsNew}
                        className="p-3 border border-dashed border-stone-800 rounded-xl text-[9px] font-black text-stone-600 hover:text-orange-400 hover:border-orange-500/20 text-center uppercase tracking-widest transition-all"
                      >
                        {t('sync.action.create_new', lang)}
                      </button>
                    </div>
                  )}
                </div>
            ...
            <div className="p-3 bg-stone-950/40 rounded-xl space-y-2">
              <span
                className={`${Styles.text.labelSm} ${isHypothetical ? 'text-indigo-400' : 'text-emerald-400'}`}
              >
                {t('sync.label.rationale', lang)}
              </span>
              <Txt variant="bodySm" className="italic">
                {String(op.rationale || '')}
              </Txt>
            </div>
            ...
            <div className="flex gap-2 pt-2">
              <button
                onClick={onReject}
                className="flex-1 py-3 bg-stone-800 text-stone-400 hover:text-white rounded-xl text-[9px] font-black uppercase transition-colors"
              >
                <X size={14} />
              </button>
              <button
              ...
              >
                {isApplying ? (
                  ...
                  <>
                    {isHypothetical ? t('sync.btn.fix', lang) : t('sync.btn.apply', lang)}{' '}
                    <ArrowRight size={14} />
                  </>
                )}
              </button>

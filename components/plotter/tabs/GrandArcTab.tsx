  ...
  const handleChapterDelete = (id: string, title: string) => {
    uiDispatch(
      Actions.openDialog({
        ...
        type: 'confirm',
        title: '章の削除',
        message: `「${title}」を削除しますか？これにより原稿データも削除されます。`,
        onConfirm: () => {
    ...
    <div className="space-y-8 animate-fade-in max-w-4xl mx-auto">
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-[10px] font-black text-stone-600 uppercase tracking-widest flex items-center gap-2">
            <Anchor size={14} /> Grand Arc / 物語の背骨
          </h3>
          <button
            onClick={() => handleConsult('グランドアーク（物語全体の大筋）')}
            className="flex items-center gap-1 text-[9px] font-black text-orange-400 hover:text-white transition-colors uppercase tracking-widest"
          >
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-end">
          <h3 className="text-[10px] font-black text-stone-600 uppercase tracking-widest flex items-center gap-2">
            <Library size={14} /> Volumes / 巻構成
          </h3>
          <button
            onClick={() => openEditor('volume')}
        ...
        <div className="grid grid-cols-1 gap-4">
          {bible.volumes.length === 0 ? (
            <div className="p-8 border border-dashed border-stone-800 rounded-2xl text-center text-stone-600 text-[11px]">
              巻構成はまだありません。
            </div>
          ) : (
            bible.volumes.map((vol, i) => (
              <div
                key={vol.id}
                className="p-4 bg-stone-900/40 border border-white/5 rounded-2xl relative group hover:border-orange-500/20 transition-all"
              >
                <div className="text-[9px] font-black text-orange-400 uppercase tracking-widest mb-1">
                  Volume {vol.order || i + 1}
                </div>
            ...
            chapters.map((ch, i) => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex gap-2">
                    ...
                    <span className="text-[8px] font-mono text-stone-600">
                      {ch.wordCount.toLocaleString()}c
                    </span>
                  </div>
                </div>
                <div className="text-sm font-bold text-stone-200">{ch.title}</div>
                <div className="text-[11px] text-stone-400 font-serif mt-1 line-clamp-3">
                  {ch.summary || 'あらすじ未設定'}
                </div>
                ...
                <div className="absolute top-4 right-4 flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                  ...
                  <button
                    onClick={() =>
                      handleConsult(`第${i + 1}章：${ch.title}\nあらすじ：${ch.summary}`)
                    }
                    className="p-2 bg-stone-800 text-stone-500 hover:bg-orange-600 hover:text-white rounded-lg shadow-lg"
                    title="この章について相談"
                  >

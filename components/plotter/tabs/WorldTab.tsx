...
export const WorldTab: React.FC = () => {
    ...
    uiDispatch(Actions.openDialog({
      ...
      onConfirm: () => {
        const pathMap: Record<ItemType, keyof WorldBible | ''> = {
          ...
          entry: 'entries', race: 'races', bestiary: 'bestiary', ability: 'abilities', theme: 'themes',
  ...
  const getLocationName = (id: string) => geography?.locations.find(l => l.id === id)?.name || "Unknown";
  const getOrgName = (id: string) => geography?.organizations.find(o => o.id === id)?.name || "Unknown";
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <WorldSection 
                title="Laws / 世界の理" icon={<Scale size={14} className="text-orange-400"/>} items={foundation.laws} 
            ...
            <WorldSection 
                title="Themes / テーマ" icon={<Lightbulb size={14} className="text-yellow-400"/>} items={knowledge.themes} 
                   ...
                   {item.motifs && item.motifs.length > 0 && (
                     <div className="flex flex-wrap gap-1 mt-1.5">
                       {item.motifs.map((m: string, i: number) => (
                         <span key={i} className="text-[7px] px-1.5 py-0.5 bg-yellow-500/10 text-yellow-300 rounded border border-yellow-500/20">{m}</span>
                       ))}
                     </div>
                   )}
                </>
        ...
        {/* Locations */}
        <WorldSection 
            title="Geography / 地理" icon={<MapIcon size={14} className="text-emerald-400"/>} items={geography.locations} 
            ...
            <>
               ...
               <p className="text-[10px] text-stone-400 font-serif leading-relaxed line-clamp-3 mb-2">{item.description}</p>
               {item.connections && item.connections.length > 0 && (
                 <div className="flex flex-wrap gap-1 mt-1 border-t border-white/5 pt-1.5">
                   ...
                   {item.connections.length > 3 && <span className="text-[8px] text-stone-600">+{item.connections.length - 3}</span>}
                 </div>
               )}
            </>
        )}/>
        ...
        {/* Organizations */}
        <WorldSection 
            title="Organizations / 組織" icon={<Landmark size={14} className="text-indigo-400"/>} items={geography.organizations} 
               ...
               {item.relations && item.relations.length > 0 && (
                 <div className="flex flex-wrap gap-1 mt-1 border-t border-white/5 pt-1.5">
                   {item.relations.slice(0, 2).map((r: any, i: number) => (
                     <Badge key={i} color={r.stance === 'Hostile' ? 'rose' : r.stance === 'Ally' ? 'emerald' : 'stone'} className="flex items-center gap-1 opacity-80">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <WorldSection 
                title="Races / 種族" icon={<Users size={14} className="text-purple-400"/>} items={knowledge.races} 
                ...
                <>
                   <div className="flex justify-between items-start mb-1">
                      <span className="text-[11px] font-bold text-stone-200">{item.name}</span>
                      {item.lifespan && <span className="text-[8px] bg-stone-800 text-stone-500 px-1.5 py-0.5 rounded font-mono">{item.lifespan}</span>}
                   </div>
                   <p className="text-[10px] text-stone-400 font-serif leading-relaxed line-clamp-2">{item.description}</p>
                   {item.traits && item.traits.length > 0 && (
                     <div className="flex flex-wrap gap-1 mt-1.5">
                       {item.traits.slice(0, 3).map((t: string, i: number) => (
                         <span key={i} className="text-[7px] px-1 py-0.5 bg-purple-500/10 text-purple-300 rounded border border-purple-500/20">{t}</span>
                       ))}
                     </div>
                   )}
                </>
            ...
            <WorldSection 
                title="Bestiary / 生物" icon={<Skull size={14} className="text-rose-400"/>} items={knowledge.bestiary} 
                ...
                <>
                   <div className="flex justify-between items-start mb-1">
                      <span className="text-[11px] font-bold text-stone-200">{item.name}</span>
                      <span className={`text-[8px] px-1.5 py-0.5 rounded uppercase font-black ${item.dangerLevel === 'Catastrophic' ? 'bg-rose-600 text-white' : item.dangerLevel === 'Deadly' ? 'bg-rose-500/20 text-rose-400' : 'bg-stone-800 text-stone-500'}`}>{item.dangerLevel}</span>
                   </div>
                   <p className="text-[10px] text-stone-400 font-serif leading-relaxed line-clamp-2">{item.description}</p>
                   {item.habitat && <div className="text-[8px] text-stone-500 mt-1 flex items-center gap-1">📍 {item.habitat}</div>}
                </>
            )}/>
            <WorldSection 
                title="Magic / 能力" icon={<Zap size={14} className="text-yellow-400"/>} items={knowledge.abilities} 
                ...
                <>
                   <div className="flex justify-between items-start mb-1">
                      <span className="text-[11px] font-bold text-stone-200">{item.name}</span>
                      <span className="text-[8px] bg-yellow-500/10 text-yellow-400 px-1.5 py-0.5 rounded uppercase font-black">{item.type}</span>
                   </div>
                   <p className="text-[10px] text-stone-400 font-serif leading-relaxed line-clamp-2">{item.description}</p>
                   {item.cost && <div className="text-[8px] text-stone-500 mt-1 flex items-center gap-1">Cost: {item.cost}</div>}
                </>
            )}/>
        </div>
        ...
        {/* Items & Entries */}
        <WorldSection 
            title="Key Items / 重要アイテム" icon={<Key size={14} className="text-amber-400"/>} items={knowledge.keyItems} 
        ...
        <WorldSection 
            title="Encyclopedia / 用語集" icon={<Book size={14} className="text-stone-400"/>} items={knowledge.entries} 
            ...
            <>
               ...
               <p className="text-[10px] text-stone-400 font-serif leading-relaxed line-clamp-3">{item.definition || item.content || item.description}</p>
            </>
        )}/>

...
export const translateKey = (key: string, lang: AppLanguage) => {
  ...
  const systemKeys: Record<string, string> = lang === 'ja' ? {
    id: "ID",
    profile: "プロフィール",
    state: "状態",
    relationships: "人間関係",
...
export const ValueRenderer: React.FC<{ value: any, depth?: number, resolver?: (id: string) => string | undefined, currentKey?: string, lang: AppLanguage }> = ({ value, depth = 0, resolver, currentKey, lang }) => {
  if (value === null || value === undefined) return <span className="text-stone-600 italic">---</span>;
  
  if (Array.isArray(value)) {
    if (value.length === 0) return <span className="text-stone-600 italic text-[10px]">(None)</span>;
    ...
    return <span className="text-[10px] font-mono text-stone-500">[List ({value.length} items)]</span>;
  }
           ...
           return (
             <div key={k} className="group">
                ...
                <div className="text-[11px] md:text-[12px] text-stone-300 leading-relaxed font-serif">
                   <ValueRenderer value={v} depth={depth + 1} resolver={resolver} currentKey={k} lang={lang} />
                </div>
             </div>
           );
         })}
  ...
  if (resolver && currentKey && idKeys.includes(currentKey) && typeof value === 'string') {
     ...
     if (name) return <span className="whitespace-pre-wrap text-emerald-400 font-bold" title={value}>{name}</span>;
  }

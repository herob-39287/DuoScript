...
export const GenericItemForm: React.FC<GenericItemFormProps> = ({ 
  type, data, context, onChange, onAutoFill, loadingField 
}) => {
  ...
  const renderField = (field: FieldSchema, currentValue: any, handleChange: (val: any) => void, pathPrefix: string) => {
      ...
      return (
        <div key={field.key} className="space-y-3 pt-2 border-t border-white/5 mt-2">
          <div className="flex items-center justify-between">
            ...
            <button onClick={handleAddItem} type="button" className="text-orange-400 hover:text-white flex items-center gap-1 text-[9px] font-black uppercase transition-colors">
              <Plus size={12}/> {t('common.add', uiLanguage)}
            </button>
          </div>
          <div className="space-y-3">
            {items.map((item: any, idx: number) => (
              <div key={idx} className="p-3 bg-stone-950/30 border border-white/5 rounded-xl space-y-3 relative">
                {field.subFields?.map(subField => (
                  <div key={subField.key}>
                    {renderField(subField, item[subField.key], (val) => handleUpdateItem(idx, subField.key, val), `${fieldKey}.${idx}.`)}
                  </div>
                ))}
                <button 
            ...
            {items.length === 0 && (
              <div className="text-center text-stone-600 text-[10px] py-2 italic border border-dashed border-stone-800 rounded-lg">List is empty</div>
            )}
          </div>
        </div>
      );
    ...
    const inputClass = "w-full bg-stone-950 border border-white/10 rounded-xl px-3 py-2 text-xs md:text-sm text-stone-200 outline-none focus:border-orange-500/50 transition-all placeholder:text-stone-700";
  ...
  return (
    <div className="space-y-4">
      {schema.map(field => renderField(field, data[field.key], (val) => onChange(field.key, val), ''))}
    </div>
  );
};
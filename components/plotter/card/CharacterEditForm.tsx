  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 pr-2">
        ...
        <div className="space-y-3">
          <div className="space-y-1 relative group">
            ...
            />
            <FillBtn
              onClick={() => handleFillWrapper('shortSummary', '一言紹介')}
              loading={loadingField === 'shortSummary'}
            />
          </div>
          <div className="relative group">
            <label className="text-[8px] font-black text-stone-600 uppercase">
              Description / 詳細
            </label>
            <textarea
            ...
            />
            <FillBtn
              onClick={() => handleFillWrapper('description', '詳細説明')}
              loading={loadingField === 'description'}
            />
          </div>
          ...
          <div className="flex items-start gap-3 relative group bg-stone-950/30 p-2 rounded-lg border border-white/5">
            ...
            <div className="space-y-0.5 w-full">
              <span className="text-[8px] font-black text-stone-600 uppercase">
                Motivation / 動機
              </span>
              <textarea
            ...
            <FillBtn
              onClick={() => handleFillWrapper('motivation', '動機')}
              loading={loadingField === 'motivation'}
            />
          </div>
        </div>
        ...
        <div className="space-y-3 pt-2">
          <h4 className="text-[9px] font-black text-stone-500 uppercase tracking-widest border-b border-white/5 pb-1">
            Deep Profile
          </h4>
          ...
          <div className="space-y-1 relative group bg-stone-950/30 p-2 rounded-lg border border-white/5">
            <div className="flex items-center gap-1.5 text-[8px] font-black text-stone-600 uppercase tracking-widest">
              <Sun size={12} /> Appearance / 外見
            </div>
            <textarea
              value={profile.appearance}
              onChange={(e) => setProfile({ ...profile, appearance: e.target.value })}
              className="w-full bg-transparent text-[10px] text-stone-300 font-serif h-16 outline-none resize-none"
              placeholder="外見..."
            />
            <FillBtn
              onClick={() => handleFillWrapper('appearance', '外見')}
              loading={loadingField === 'appearance'}
            />
          </div>
          <div className="space-y-1 relative group bg-stone-950/30 p-2 rounded-lg border border-white/5">
            <div className="flex items-center gap-1.5 text-[8px] font-black text-stone-600 uppercase tracking-widest">
              <Brain size={12} /> Personality / 性格
            </div>
            <textarea
            ...
            />
            <FillBtn
              onClick={() => handleFillWrapper('personality', '性格')}
              loading={loadingField === 'personality'}
            />
          </div>
          <div className="space-y-1 relative group bg-stone-950/30 p-2 rounded-lg border border-white/5">
            <div className="flex items-center gap-1.5 text-[8px] font-black text-stone-600 uppercase tracking-widest">
              <Moon size={12} /> Background / 背景
            </div>
            <textarea
            ...
            />
            <FillBtn
              onClick={() => handleFillWrapper('background', '背景・生い立ち')}
              loading={loadingField === 'background'}
            />
          </div>
          <div className="space-y-1 relative group bg-stone-950/30 p-2 rounded-lg border border-white/5">
            <div className="flex items-center gap-1.5 text-[8px] font-black text-stone-600 uppercase tracking-widest">
              <Skull size={12} /> Flaw / 欠点
            </div>
            <textarea
              value={profile.flaw}
              onChange={(e) => setProfile({ ...profile, flaw: e.target.value })}
              className="w-full bg-transparent text-[10px] text-stone-300 font-serif h-12 outline-none resize-none"
              placeholder="致命的な欠点..."
            />
            <FillBtn
              onClick={() => handleFillWrapper('flaw', '欠点')}
              loading={loadingField === 'flaw'}
            />
          </div>
          <div className="space-y-1 relative group bg-stone-950/30 p-2 rounded-lg border border-white/5">
            <div className="flex items-center gap-1.5 text-[8px] font-black text-stone-600 uppercase tracking-widest">
              <Wand2 size={12} /> Character Arc / アーク
            </div>
            <textarea
              value={profile.arc}
              onChange={(e) => setProfile({ ...profile, arc: e.target.value })}
              className="w-full bg-transparent text-[10px] text-stone-300 font-serif h-12 outline-none resize-none"
              placeholder="成長・変化の軌跡..."
            />
            <FillBtn
              onClick={() => handleFillWrapper('arc', 'キャラクターアーク')}
              loading={loadingField === 'arc'}
            />
          </div>
        </div>
        ...
        <div className="space-y-3 pt-2">
          <div className="flex items-center justify-between border-b border-white/5 pb-1">
            <h4 className="text-[9px] font-black text-stone-500 uppercase tracking-widest">
              Relationships
            </h4>
            <button
              onClick={handleAddRelationship}
              className="text-orange-400 hover:text-white flex items-center gap-1 text-[8px] font-black uppercase"
            >
              <Plus size={10} /> Add
            </button>
          </div>
          <div className="space-y-2">
            {relationships.map((rel, idx) => (
              <div
                key={idx}
                className="p-3 bg-stone-950/30 border border-white/5 rounded-xl space-y-2 relative"
              >
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      className="w-full bg-stone-950 text-xs text-stone-300 rounded px-2 py-1 outline-none border border-white/10 appearance-none"
                    >
                      {['Ally', 'Enemy', 'Romance', 'Family', 'Business', 'Other', 'Complex'].map(
                        (t) => (
                ...
                <button
                  ...
                  className="absolute -top-2 -right-2 p-1 bg-stone-800 text-stone-500 hover:text-rose-400 rounded-full shadow-lg"
                >
                  <X size={12} />
                </button>
              </div>
      ...
      <div className="pt-4 mt-2 border-t border-white/5 flex gap-2 shrink-0">
        ...
        <Button
          variant="primary"
          onClick={() => onSave(profile, relationships)}
          icon={<Save size={12} />}
          className="flex-1"
        >
          保存して終了

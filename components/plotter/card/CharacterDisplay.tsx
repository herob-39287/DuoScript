...
const DetailSection = ({
  ...
  <div className="space-y-1">
    ...
    <p className="text-[10px] text-stone-400 font-serif leading-relaxed whitespace-pre-wrap">
      {content}
    </p>
  </div>
);
    ...
    <div className="flex-1 flex flex-col h-full">
        ...
        <div className="space-y-3 pt-2">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-[10px] text-stone-300 font-serif leading-relaxed line-clamp-2">
                {c.profile.motivation || '未設定'}
              </p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-[10px] text-stone-400 font-serif italic">
                @{c.state.location || '不明'}: {c.state.internalState || '平常'}
              </p>
            </div>
          </div>
        </div>
        ...
        {isExpanded && (
          <div className="pt-4 border-t border-white/5 space-y-4 animate-fade-in flex-1 overflow-hidden flex flex-col">
            <div className="flex gap-2 border-b border-white/5 pb-2 shrink-0">
              <Button
                variant={activeDetailTab === 'profile' ? 'primary' : 'ghost'}
                size="xs"
                onClick={() => setActiveDetailTab('profile')}
                className={
                  activeDetailTab === 'profile'
                    ? 'bg-orange-600/20 text-orange-400 shadow-none'
                    : ''
                }
              >
                プロフィール詳細
              ...
              <Button
                variant={activeDetailTab === 'relationships' ? 'primary' : 'ghost'}
                size="xs"
              ...
              {activeDetailTab === 'profile' && (
                <>
                  <DetailSection
                    icon={<Sun size={12} />}
                    label="Appearance / 外見"
                    content={c.profile.appearance || '外見の描写はありません。'}
                  />
                  <DetailSection
                    icon={<Brain size={12} />}
                    label="Personality / 性格"
                    content={c.profile.personality || '性格の定義はありません。'}
                  />
                  <DetailSection
                    icon={<Moon size={12} />}
                    label="Background / 背景"
                    content={c.profile.background || '背景ストーリーはありません。'}
                  />
                  <DetailSection
                    icon={<Skull size={12} />}
                    label="Flaw / 欠点"
                    content={c.profile.flaw || '致命的な欠点はありません。'}
                  />
                  <DetailSection
                    icon={<Wand2 size={12} />}
                    label="Character Arc / アーク"
                    content={c.profile.arc || 'アークの定義はありません。'}
                  />

                  {c.profile.voice && (
                    <div className="p-3 bg-stone-900/50 rounded-xl space-y-1">
                      <div className="flex items-center gap-1.5 text-[8px] font-black text-stone-600 uppercase tracking-widest">
                        <MessageCircle size={10} /> Voice / 口調
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-2">
                    <StatusItem
                      icon={<MapPin size={12} />}
                      label="Location / 場所"
                      value={c.state.location || '不明'}
                    />
                    <StatusItem
                      icon={<Heart size={12} />}
                      label="Health / 健康"
                      value={c.state.health || '良好'}
                    />
                  </div>
                  <div className="space-y-2 pt-2">
                    <div className="flex items-center gap-2 text-[8px] font-black text-stone-600 uppercase tracking-widest">
                      <Package size={12} /> Key Items / 所持品
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      {inventory.length === 0 ? (
                        <span className="text-[9px] text-stone-700 italic">なし</span>
                      ) : (
                        inventory.map((item, i) => (
                          <Badge key={i} color="stone">
                            {item}
                          </Badge>
                        ))
                      )}
                ...
                <div className="space-y-3">
                  {c.relationships && c.relationships.length > 0 ? (
                    c.relationships.map((rel, idx) => (
                      <div
                        key={idx}
                        className="p-3 bg-stone-900/50 border border-white/5 rounded-xl space-y-1"
                      >
                        <div className="flex justify-between items-center">
                          <span className="text-[10px] font-bold text-stone-200">
                            {getRelationshipName(rel.targetId)}
                          </span>
                    ...
                    <div className="text-center text-stone-600 text-[10px] italic py-10">
                      関係性の定義はありません。
                    </div>
                  )}
                </div>
      ...
      <div className="flex items-center justify-between pt-4 mt-auto border-t border-white/5 shrink-0">
        <Button
          ...
          className="text-[8px] md:text-[8px] px-0 hover:bg-transparent"
        >

...
export const CharacterCard = React.memo(
  ({ character: c, onGeneratePortrait, isGenerating }: CharacterCardProps) => {
    ...
    const { handleSave, handleDelete, handleConsult, handleGenesisFill, loadingField } =
      useCharacterActions(c);
    ...
    const inventory = bible.keyItems.filter((i) => i.currentOwnerId === c.id).map((i) => i.name);
    ...
    return (
      <Card
        ...
        padding="none"
        className={`group ${isEditing ? 'border-orange-500/40' : ''} flex flex-col h-[500px] lg:h-[600px] transition-all`}
      >
        {/* Header / Image Area */}
        <div
          className={`relative bg-stone-900 transition-all duration-500 ${isEditing ? 'h-24 shrink-0' : 'h-2/5 shrink-0'}`}
        >
          {isImageLoading ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center gap-2 text-stone-700">
              ...
              {!isEditing && (
                <span className="text-[8px] font-black uppercase tracking-[0.2em] text-rose-900/50">
                  Load Failed
                </span>
              )}
            </div>
            ...
            <img
              src={imageUrl}
              className={`w-full h-full object-cover transition-transform duration-1000 ${isEditing ? 'opacity-50' : 'group-hover:scale-105'}`}
            />
          ) : (
          ...
          {/* Overlay Actions */}
          <div
            className={`absolute top-4 right-4 flex gap-2 transition-opacity ${isEditing ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
          >
            {!isEditing && (
              <>
                <Button
          ...
          {!isEditing && (
            <div className="absolute bottom-4 left-6 bg-stone-950/60 backdrop-blur-md px-4 py-2 rounded-xl flex items-center gap-2 max-w-[80%]">
              <h4 className="text-base md:text-lg font-display font-black text-white italic truncate">
                {c.profile.name}
              </h4>

  ...
  return (
    <div className="fixed inset-0 bg-stone-900 flex flex-col items-center p-4 md:p-12 overflow-y-auto no-scrollbar pb-safe">
      <div className="absolute top-4 right-4 z-50 flex items-center gap-2">
        <div
          className={`flex items-center gap-1.5 px-2 py-1 rounded-full border text-[9px] font-black uppercase tracking-widest ${hasApiKey ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-rose-500/10 border-rose-500/20 text-rose-400'}`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-3 py-1.5 rounded-full bg-stone-800 text-stone-400 hover:text-white transition-all border border-white/5 ${Styles.text.label}`}
        >
      ...
      <div className="w-full max-w-4xl space-y-8 md:space-y-16 animate-fade-in py-6 md:py-12">
        <header className="text-center space-y-4 md:space-y-6">
          ...
          <p className="text-stone-500 font-serif italic text-xs md:text-lg tracking-[0.2em] md:tracking-widest px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
            {isLoadingProjects ? (
              <Card
                variant="panel"
                padding="lg"
                className="col-span-1 md:col-span-2 flex items-center justify-center text-stone-700 gap-3"
              >
              ...
              projects.map((project) => (
                ...
                >
                  <Card
                    variant="panel"
                    padding="md"
                    className="group-hover:bg-stone-800 transition-colors h-full"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="font-display font-black text-sm md:text-lg text-stone-200 italic truncate">
                            {project.meta.title}
                          </div>
                          <div className={`${Styles.text.labelSm} mt-0.5 truncate`}>
                            V{project.bible.version} /{' '}
                      ...
                      <ChevronRight
                        size={18}
                        className="text-stone-600 group-hover:text-orange-400 transition-colors shrink-0"
                      />
                    </div>
                  </Card>
                </button>
              ...
              <Card
                variant="outline"
                padding="lg"
                className="col-span-1 md:col-span-2 flex items-center justify-center text-stone-700 italic text-[10px] font-serif"
              >
            ...
            <label className="block w-full focus-within:ring-2 focus-within:ring-orange-400 rounded-[1.5rem] md:rounded-[2rem] cursor-pointer">
              <Card
                variant="panel"
                padding="md"
                className="h-full flex items-center gap-4 hover:bg-stone-800/40 hover:border-orange-500/30 group transition-colors"
              >
                ...
                <div className="text-left">
                  <div className={`${Styles.text.labelSm} text-stone-400`}>
                    {t('welcome.load_backup', lang)}
                  </div>
                  <div className="text-[8px] md:text-[9px] text-stone-700 mt-0.5 truncate">
                    {t('welcome.load_json', lang)}
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 pb-10">
            <Card variant="glass-bright" padding="lg" className="flex flex-col gap-6 shadow-2xl">
              <div className="flex items-center gap-3 md:gap-4">
                <div className="p-2 md:p-3 bg-orange-400/20 rounded-2xl text-orange-400">
                  <Coffee size={18} className="md:w-6 md:h-6" />
                </div>
              ...
              <div className="space-y-3 md:space-y-4 flex-1">
                <input
                  ...
                  placeholder={t('welcome.manual_desc', lang)}
                  className="w-full bg-stone-900/50 border border-stone-700 rounded-xl md:rounded-2xl p-4 md:p-5 text-sm text-white outline-none focus:border-orange-400/50 transition-all"
                />
                <textarea
                  value={idea}
                  onChange={(e) => setIdea(e.target.value)}
                  placeholder={t('welcome.manual_idea', lang)}
                  className="w-full bg-stone-900/50 border border-stone-700 rounded-xl md:rounded-2xl p-4 md:p-5 text-sm text-white h-24 md:h-36 outline-none resize-none focus:border-orange-400/50 transition-all font-serif"
                />
              </div>
              <Button
                variant="primary"
                onClick={handleLaunchManual}
                className="w-full uppercase tracking-widest text-[10px] md:text-xs py-4 md:py-5"
              >
                {t('welcome.enter_atelier', lang)}
              </Button>
            </Card>
            ...
            <Card
              ...
              padding="lg"
              className="flex flex-col gap-6 shadow-2xl border-orange-400/10"
            >
              <div className="flex items-center gap-3 md:gap-4">
                <div className="p-2 md:p-3 bg-stone-700/50 rounded-2xl text-orange-300">
                  <Rocket size={18} className="md:w-6 md:h-6" />
                </div>
              ...
              <div className="flex-1 space-y-4 md:space-y-6">
                <p className="text-[11px] md:text-sm text-stone-500 font-serif italic leading-relaxed">
                  {t('welcome.muse_desc', lang)}
                </p>
                <input
                  value={autoTheme}
                  onChange={(e) => setAutoTheme(e.target.value)}
                  placeholder={t('welcome.muse_placeholder', lang)}
                  className="w-full bg-stone-900/50 border border-stone-700 rounded-xl md:rounded-2xl p-4 md:p-5 text-sm text-white outline-none focus:border-orange-400/50 transition-all"
                />
              </div>
              <Button
                variant="ghost"
                onClick={handleLaunchAuto}
                isLoading={isProcessing}
                icon={<Sparkles size={16} />}
                className="w-full !bg-stone-100 hover:!bg-white !text-stone-900 uppercase tracking-widest text-[10px] md:text-xs py-4 md:py-5 flex-col gap-1 h-auto"
              >
                {isProcessing ? (
                  <>
                    <span>{progressMsg}</span>
                    <span className="text-[8px] font-normal opacity-60 normal-case">
                      (完了まで数分かかります)
                    </span>
                  </>
                ) : (

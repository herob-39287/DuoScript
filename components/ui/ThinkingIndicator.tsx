...
export const ThinkingIndicator: React.FC<ThinkingIndicatorProps> = ({ phase }) => {
  ...
  if (phase.includes('検索') || phase.includes('Looking') || phase.includes('Search'))
    icon = <Search size={14} className="animate-bounce text-sky-400" />;
  else if (phase.includes('執筆') || phase.includes('Drafting') || phase.includes('Writing'))
    icon = <Edit3 size={14} className="animate-pulse text-emerald-400" />;
  else if (phase.includes('整理') || phase.includes('Memory') || phase.includes('Saving'))
    icon = <Database size={14} className="animate-spin text-purple-400" />;

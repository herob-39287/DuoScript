      ...
      <div className="bg-stone-900 w-full max-w-md rounded-3xl border border-stone-800 shadow-3xl overflow-hidden flex flex-col scale-in">
        <div className="p-8 space-y-4">
          <div className="flex items-center gap-4">
            <div className={`p-3 rounded-2xl ${dialog.type === 'confirm' ? 'bg-orange-600/20 text-orange-400' : 'bg-rose-600/20 text-rose-400'}`}>
              {dialog.type === 'confirm' ? <HelpCircle size={24} /> : <AlertCircle size={24} />}
            </div>
            <h3 className="text-xl font-black text-white italic tracking-tighter uppercase">{dialog.title}</h3>
          </div>

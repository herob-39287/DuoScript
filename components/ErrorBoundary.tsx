...
class ErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  private handleEmergencyReset = () => {
    if (
      window.confirm(
        '現在開いているプロジェクトの表示設定をリセットしてトップに戻りますか？データ自体は削除されません。',
      )
    ) {
          ...
          <div className="max-w-md w-full bg-stone-900 border border-stone-800 rounded-3xl p-8 shadow-2xl space-y-6">
            ...
            <div className="space-y-3">
              <button
                ...
                className="w-full py-4 bg-orange-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-orange-500 shadow-lg shadow-orange-900/20 transition-all active:scale-95 flex items-center justify-center gap-2"
              >
                <RefreshCw size={14} /> 再試行
              </button>
              <button
                onClick={this.handleReload}
                className="w-full py-4 bg-stone-800 text-stone-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:text-white transition-all active:scale-95"
              >
                リロード
              </button>
              <button
                onClick={this.handleEmergencyReset}
                className="w-full py-4 bg-rose-900/20 text-rose-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-900/40 transition-all active:scale-95 flex items-center justify-center gap-2"
              >

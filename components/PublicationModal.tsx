...
import { X, Check, BookOpen, FileJson, Eye, Settings2, Loader2, AlertCircle, FolderArchive } from 'lucide-react';
import { loadFullSnapshot, getHeadRev, getAllAssetMetadata, getPortrait } from '../services/storageService';
            ...
            exportData.bible.characters.forEach(c => {
                ...
                if (c.relationships && c.relationships.length > 0) {
                    ...
                    c.relationships.forEach(r => {
                        const targetName = exportData?.bible.characters.find(t => t.id === r.targetId)?.profile.name || "Unknown";
                    ...
                    <div className={`${mobileTab === 'settings' ? 'flex' : 'hidden md:flex'} w-full md:w-80 border-b md:border-b-0 md:border-r border-stone-800 p-6 space-y-6 overflow-y-auto bg-stone-900/20 shrink-0 custom-scrollbar`}>
                        ...
                        <div className="space-y-4 pb-20 md:pb-0">
                            ...
                            <div className="grid grid-cols-1 gap-2">
                                <button onClick={handleDownloadMarkdownZip} disabled={!exportData || isPreparing} className="w-full py-4 bg-stone-800 hover:bg-stone-700 text-stone-200 rounded-xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest transition-all shadow-lg active:scale-95 disabled:opacity-40">
                                    <FolderArchive size={14}/> Markdown一括書出し (.zip)
                                </button>
                                <button onClick={handleDownloadFullBackup} disabled={!exportData || isPreparing} className="w-full py-4 bg-stone-800 hover:bg-stone-700 text-stone-200 rounded-xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest transition-all shadow-lg active:scale-95 disabled:opacity-40">

...
const StatCard = React.memo(({ icon, label, value, unit, color }: StatCardProps) => (
  <Card
    variant="glass"
    padding="md"
    className="h-24 md:h-40 flex flex-col justify-between relative overflow-hidden group min-w-0"
  >
    <div className="absolute top-2 right-2 md:top-8 md:right-8 text-white/5 group-hover:text-white/10 transition-colors scale-75 md:scale-100">
      {icon}
    </div>
    <Txt variant="labelSm" className="truncate">
      {label}
    </Txt>

...
export const SafetyMonitor: React.FC<SafetyMonitorProps> = ({ violationCount, violationHistory, lang }) => {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
       <Card variant="glass" padding="lg" className="lg:col-span-4 space-y-6 mx-1 border border-amber-500/10 min-w-0">
          <SectionHeader icon={<AlertTriangle size={20} className="text-amber-500" />} title={t('dashboard.safety_status', lang)} />
          ...
          <p className={Styles.text.body}>{violationCount > 0 ? "Safety policy triggered." : "All green."}</p>
       </Card>
                ...
                <div key={i} className="p-3 md:p-4 bg-stone-950/40 rounded-2xl border border-white/5 flex items-center justify-between gap-4">
                   <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                         ...
                         <span className={Styles.text.mono}>{new Date(v.timestamp).toLocaleTimeString()}</span>
                      </div>
                      <p className="text-[10px] text-stone-500 italic truncate">"{v.inputSnippet}..."</p>
                   </div>
                </div>
             )) : (
                <div className="h-full flex items-center justify-center text-stone-700 italic font-serif text-xs py-8">No violations recorded.</div>
             )}
          </div>
       </Card>
    </div>

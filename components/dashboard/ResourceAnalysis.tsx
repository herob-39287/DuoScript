...
export const ResourceAnalysis: React.FC<ResourceAnalysisProps> = ({
  data,
  viewMode,
  onViewModeChange,
  lang,
        ...
        <div className="flex flex-col md:flex-row items-center gap-6">
          <div className="p-1.5 bg-stone-950/60 rounded-xl border border-white/5 flex gap-1 w-full md:w-auto">
            <Button
              ...
              className={viewMode === 'source' ? 'bg-stone-800 text-white shadow-lg' : ''}
            >
              Source
            </Button>
            <Button
              variant={viewMode === 'model' ? 'primary' : 'ghost'}
              size="sm"
              icon={<Cpu size={12} />}
              onClick={() => onViewModeChange('model')}
              className={viewMode === 'model' ? 'shadow-lg' : ''}
            >
              Model
            </Button>
            <Button
              variant={viewMode === 'architect' ? 'indigo' : 'ghost'}
              size="sm"
              icon={<BrainCircuit size={12} />}
              onClick={() => onViewModeChange('architect')}
              className={viewMode === 'architect' ? 'shadow-lg' : ''}
            >
              Architect
            </Button>
          </div>
          <div className="flex gap-4 self-end md:self-center">
            <div className="flex flex-col items-end">
              <Txt variant="labelSm" className="flex items-center gap-1">
                <ArrowDownLeft size={10} className="text-emerald-500" /> Input
              </Txt>
              <span className="text-base md:text-lg font-mono font-black text-emerald-400">
                {data.reduce((acc, i) => acc + i.input, 0).toLocaleString()}
              </span>
            </div>
            <div className="flex flex-col items-end">
              <Txt variant="labelSm" className="flex items-center gap-1">
                <CloudLightning size={10} className="text-sky-400" /> Cached
              </Txt>
              <span className="text-base md:text-lg font-mono font-black text-sky-400">
                {data.reduce((acc, i) => acc + i.cached, 0).toLocaleString()}
              </span>
            </div>
            <div className="flex flex-col items-end">
              <Txt variant="labelSm" className="flex items-center gap-1">
                <ArrowUpRight size={10} className="text-orange-400" /> Output
              </Txt>
              <span className="text-base md:text-lg font-mono font-black text-orange-400">
                {data.reduce((acc, i) => acc + i.output, 0).toLocaleString()}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-2 md:space-y-3 max-h-[250px] md:max-h-[300px] overflow-y-auto no-scrollbar pr-2 min-w-0">
          ...
          {data.map((item, idx) => (
            <div
              key={idx}
              className="flex flex-col p-2.5 md:p-3 bg-stone-950/40 rounded-xl border border-white/5 gap-2"
            >
              <div className="flex items-center gap-3 min-w-0">
                <div
                  className="w-1.5 h-1.5 rounded-full shrink-0"
                  style={{ backgroundColor: getBarColor(item.label, idx) }}
                />
                <span className="text-[10px] font-black text-stone-300 uppercase truncate">
                  {item.label}
                </span>
              </div>
              <div className="flex justify-between items-center pl-4">
                <span className="text-[9px] text-stone-500">
                  Eff: {Math.round((item.cached / (item.input + item.cached || 1)) * 100)}%
                </span>

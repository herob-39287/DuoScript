...
const LogItem = React.memo(({ log }: { log: SystemLog }) => (
  <Card
    ...
    padding="sm"
    className={`border-l-2 animate-fade-in ${log.type === 'error' ? 'border-l-rose-500' : log.type === 'success' ? 'border-l-emerald-500' : 'border-l-orange-400/50'}`}
  >
    <div className="flex gap-3 items-start">
      <div className="px-1.5 py-0.5 rounded-[4px] text-[8px] font-black uppercase bg-stone-800 text-stone-500 shrink-0">
        {log.source}
      </div>
...
export const AtelierLog: React.FC<AtelierLogProps> = ({
  logs,
  summaryBuffer,
  onClearLogs,
  lang,
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-10 px-1">
      <Card
        variant="glass"
        padding="lg"
        className="lg:col-span-8 flex flex-col h-[400px] md:h-[600px] shadow-2xl relative min-w-0"
      >
        <SectionHeader
          ...
          action={
            <button
              onClick={onClearLogs}
              className="p-2 hover:bg-stone-800 rounded-lg text-stone-700 hover:text-rose-400 transition-colors"
            >
              <Trash2 size={16} />
            </button>
          }
      ...
      <Card
        variant="glass"
        padding="lg"
        className="lg:col-span-4 flex flex-col justify-between h-auto md:h-full min-h-[200px] relative overflow-hidden shadow-2xl min-w-0"
      >
        <BrainCircuit
          size={100}
          className="absolute -bottom-4 -right-4 text-white/[0.02] rotate-12"
        />

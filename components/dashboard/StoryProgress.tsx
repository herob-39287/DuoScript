  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 px-1">
      <Card
        variant="glass"
        padding="lg"
        className="lg:col-span-2 space-y-6 h-[300px] md:h-[400px] flex flex-col min-w-0"
      >
        <div className="flex items-center justify-between">
          <SectionHeader
            icon={<BarChart3 size={16} className="text-orange-400" />}
            title={t('dashboard.progress', lang)}
          />
          <span className="text-[10px] font-mono text-stone-700">
            {totalWordCount.toLocaleString()} c
          </span>
        </div>
      ...
      <Card
        variant="glass"
        padding="lg"
        className="space-y-6 h-[300px] md:h-[400px] flex flex-col min-w-0"
      >
        <SectionHeader
          icon={<Activity size={16} className="text-indigo-400" />}
          title={t('dashboard.roles', lang)}
        />
        ...
        <div className="grid grid-cols-2 gap-2 overflow-y-auto no-scrollbar">
          {roleDistribution.slice(0, 4).map((r, i) => (
            <div key={r.name} className="flex items-center gap-2">
              ...
              />
              <span className="text-[10px] font-black text-stone-500 uppercase truncate">
                {r.name}
              </span>
            </div>
          ))}

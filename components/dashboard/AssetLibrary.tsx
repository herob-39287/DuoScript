...
const AssetThumbnail: React.FC<{ asset: AssetMetadata; onDelete: () => void }> = ({ asset, onDelete }) => {
...
export const AssetLibrary: React.FC<AssetLibraryProps> = ({ assets, onDeleteAsset, lang }) => {
  return (
    <Card variant="glass" padding="lg" className="space-y-6 md:space-y-8 mx-1 min-w-0">
      <SectionHeader icon={<ImageIcon size={20} className="text-orange-400" />} title={t('dashboard.assets_library', lang)} />
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-3 md:gap-4">
        {assets.length === 0 ? (
          <div className="col-span-full py-8 text-center text-stone-700 italic font-serif border border-dashed border-stone-800 rounded-2xl">Empty</div>
        ) : (
          assets.map(asset => (

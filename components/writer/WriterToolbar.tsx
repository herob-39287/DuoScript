...
import { Sparkles, Loader2, Feather, ScanSearch, Minimize2, Book, ChevronRight, ChevronLeft, PenTool, WifiOff, FileSearch } from 'lucide-react';
...
export const WriterToolbar: React.FC<WriterToolbarProps> = ({
  ...
  if (!isExpanded) {
    return (
      <div className={`fixed ${isZenMode ? 'bottom-8' : 'bottom-24 md:bottom-10'} right-4 md:right-auto md:left-1/2 md:-translate-x-1/2 z-50 transition-all`}>
        <button 
  ...
  return (
    <div className={`fixed ${isZenMode ? 'bottom-8' : 'bottom-24 md:bottom-10'} left-1/2 -translate-x-1/2 flex items-center gap-2 md:gap-4 transition-all duration-700 z-50 w-auto max-w-[95vw]`}>
      ...
      <div className={`glass p-1.5 md:p-2 rounded-2xl md:rounded-[2rem] border border-white/10 flex items-center gap-1 md:gap-4 shadow-3xl bg-stone-900/90 backdrop-blur-xl relative overflow-hidden transition-opacity duration-500 ${isZenMode && !isProcessing ? 'opacity-30 hover:opacity-100' : 'opacity-100'}`}>
        {!ui.isOnline && (
          <div className="absolute inset-0 bg-stone-950/80 backdrop-blur-sm z-50 flex items-center justify-center gap-2 cursor-not-allowed">
             ...
             <span className="text-[10px] font-black uppercase tracking-widest text-stone-500">Offline Mode</span>
          </div>
        )}
        ...
        <button 
          ...
          title="提案 (AI)"
        >
          {isSuggesting ? <Loader2 size={18} className="animate-spin" /> : <Sparkles size={18} className="group-hover:rotate-12 transition-transform" />}
          <span className="hidden sm:block text-[8px] md:text-[10px] font-black uppercase tracking-widest">{isSuggesting ? '提案中...' : '提案'}</span>
        </button>
        ...
        >
          ...
          <span className="hidden sm:block text-[8px] md:text-[10px] font-black uppercase tracking-widest">{isProcessing ? '執筆中...' : '執筆'}</span>
        </button>
        ...
        >
          ...
          <span className="hidden sm:block text-[8px] md:text-[10px] font-black uppercase tracking-widest">{isWhispering ? '確認中...' : '整合'}</span>
        </button>
        ...
        {onScan && (
          ...
          >
            ...
            <span className="hidden sm:block text-[8px] md:text-[10px] font-black uppercase tracking-widest">{isScanning ? '抽出中...' : '抽出'}</span>
          </button>
        )}
      </div>

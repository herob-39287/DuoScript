...
export const EditorCanvas: React.FC<EditorCanvasProps> = ({
  ...
  const bgColor =
    settings.paperFilter === 'dark'
      ? '#000'
      : settings.paperFilter === 'sepia'
        ? '#1f1c1a'
        : 'transparent';
  const textColor =
    settings.paperFilter === 'dark'
  ...
  return (
    ...
    >
      <div
        className={`relative transition-all duration-500 ${isVertical ? 'h-full py-8 px-20' : 'w-full max-w-3xl py-10 md:py-20'}`}
      >
        ...
        {isProcessing && (
          <div
            className={`absolute z-20 flex flex-col items-center gap-2 animate-fade-in pointer-events-none ${isVertical ? 'top-1/2 right-12 -translate-y-1/2' : 'bottom-12 left-1/2 -translate-x-1/2'}`}
          >
            <div className="px-5 py-3 bg-stone-900/90 backdrop-blur-xl border border-orange-500/30 rounded-2xl shadow-2xl shadow-orange-900/40 flex items-center gap-4">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[8px] text-orange-400 font-serif">物語を紡いでいます</span>
              </div>
            </div>
          </div>
        )}

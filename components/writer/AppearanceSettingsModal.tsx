    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-stone-950/80 backdrop-blur-sm animate-fade-in">
      <div className="bg-stone-900 w-full max-w-sm rounded-2xl border border-stone-800 shadow-3xl overflow-hidden">
        <div className="p-4 border-b border-white/5 flex justify-between items-center bg-stone-900/50">
          <span className="text-sm font-black text-stone-200 uppercase tracking-widest">
            Editor Appearance
          </span>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-3">
            <div className="text-[10px] font-black text-stone-500 uppercase tracking-widest flex items-center gap-2">
              <Palette size={12} /> 用紙テーマ
            </div>
            <div className="grid grid-cols-3 gap-2">
              <button
                onClick={() => update({ paperFilter: 'none' })}
                className={`h-10 rounded-lg border transition-all ${settings.paperFilter === 'none' ? 'border-orange-500 ring-2 ring-orange-500/20' : 'border-stone-700'} bg-[#1c1917]`}
              ></button>
              <button
                onClick={() => update({ paperFilter: 'sepia' })}
                className={`h-10 rounded-lg border transition-all ${settings.paperFilter === 'sepia' ? 'border-orange-500 ring-2 ring-orange-500/20' : 'border-stone-700'} bg-[#292524]`}
                style={{ filter: 'sepia(0.2)' }}
              ></button>
              <button
                onClick={() => update({ paperFilter: 'dark' })}
                className={`h-10 rounded-lg border transition-all ${settings.paperFilter === 'dark' ? 'border-orange-500 ring-2 ring-orange-500/20' : 'border-stone-700'} bg-black`}
              ></button>
            </div>
          </div>
        </div>

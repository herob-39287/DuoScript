      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
        {onClose && (
          <div className="flex justify-end lg:hidden">
            <button
              onClick={onClose}
              className="p-2 bg-stone-800 text-stone-500 rounded-full hover:bg-stone-700 transition-colors"
            >
              <X size={16} />
            </button>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 justify-between">
                    <span
                      className={`text-[8px] font-black uppercase ${isThisBeatProcessing ? 'text-orange-400' : 'text-stone-700'}`}
                    >
                      Beat {i + 1}
                    </span>
                    {onDraftBeat && (
                  ...
                  <p
                    className={`text-[11px] font-serif leading-relaxed line-clamp-2 group-hover:line-clamp-none transition-all cursor-text select-text ${isThisBeatProcessing ? 'text-stone-200' : 'text-stone-400'}`}
                  >
                    "{beat.text}"
                  </p>
                </div>
        ...
        {whisper && (
          <div className="space-y-4 animate-fade-in">
            ...
            <div
              className={`p-6 rounded-[2rem] border space-y-4 ${whisper.type === 'alert' ? 'bg-rose-500/5 border-rose-500/20' : 'bg-emerald-500/5 border-emerald-500/20'}`}
            >
              <div className="flex items-center gap-2">
                <span
                  className={`text-[8px] font-black px-1.5 py-0.5 rounded uppercase ${whisper.type === 'alert' ? 'bg-rose-500/20 text-rose-400' : 'bg-emerald-500/20 text-emerald-400'}`}
                >
                  {whisper.ruleId}
                </span>
              </div>
              <p className="text-[13px] text-stone-200 font-serif leading-relaxed italic">
                "{whisper.text}"
              </p>
              <div className="space-y-2 pt-2 border-t border-white/5">
                ...
                {whisper.citations.map((c: any, i: number) => (
                  <div
                    key={i}
                    className="text-[10px] text-stone-500 font-serif border-l border-stone-800 pl-2"
                  >

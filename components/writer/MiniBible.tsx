...
import { Search, User, MapPin, Package, Book, ChevronRight, X, Reply, FileText } from 'lucide-react';
...
export const MiniBible: React.FC<MiniBibleProps> = ({ onInsert, className = "" }) => {
  ...
  const [selectedItem, setSelectedItem] = useState<{ id: string; type: string; data: any } | null>(null);
    ...
    if (type === 'char') {
      ...
      content = (
        <div className="space-y-3">
          <div className="text-[10px] text-stone-400">{data.profile.shortSummary || data.profile.description}</div>
          ...
          <div className="text-[10px] text-stone-400 font-serif whitespace-pre-wrap">{data.profile.appearance}</div>
        </div>
      );
    } else if (type === 'loc') {
      ...
      content = (
        <div className="space-y-3">
           <div className="text-[10px] text-stone-400 font-serif whitespace-pre-wrap">{data.description}</div>
           {data.connections && data.connections.length > 0 && (
             <div className="text-[9px] text-stone-500">
               接続: {data.connections.length}箇所
             </div>
           )}
        </div>
      );
    ...
    return (
      <div className="absolute inset-0 bg-stone-900 z-10 flex flex-col animate-fade-in">
        <div className="flex items-center justify-between p-4 border-b border-white/5 bg-stone-800/50">
           ...
           <button onClick={() => setSelectedItem(null)} className="text-stone-500 hover:text-white shrink-0"><X size={16}/></button>
        </div>
        ...
        <>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
              ...
              >
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                     <div className={`p-1.5 rounded-lg shrink-0 ${item.type === 'char' ? 'bg-indigo-500/10 text-indigo-400' : item.type === 'loc' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-stone-800 text-stone-500'}`}>
                        {item.type === 'char' && <User size={12}/>}
                        {item.type === 'loc' && <MapPin size={12}/>}
                        {item.type === 'item' && <Package size={12}/>}
                        {item.type === 'entry' && <Book size={12}/>}
                     </div>
                     <span className="text-[11px] text-stone-300 font-medium group-hover:text-white truncate">{item.name}</span>
                  </div>
                  <ChevronRight size={14} className="text-stone-700 group-hover:text-stone-400 ml-2 shrink-0"/>
                </button>
                
                {onInsert && (
                  <button 
                    onClick={(e) => { e.stopPropagation(); handleInsert(item.name); }}
            ...
            {filteredItems.length === 0 && (
              <div className="text-center py-8 text-stone-600 text-[10px] italic">No results found.</div>
            )}
          </div>
        </>
      )}

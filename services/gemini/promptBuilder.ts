...
export class PromptBuilder {
  ...
  static buildIndexData(project: StoryProject): string {
    ...
    const index = {
      ...
      tone: bible.tone,
      laws: bible.laws.map((l) => ({
        id: l.id,
        name: l.name,
        type: l.type,
        importance: l.importance,
        summary: l.shortSummary || l.description.slice(0, 100),
      })),
  ...
  static buildFocalData(
    ...
    if (activeChapter) {
      ...
      (activeChapter.foreshadowingLinks || []).forEach((link) =>
        focalIds.add(link.foreshadowingId),
      );
    }
  ...
  static buildStructuredStoryData(
    project: StoryProject,
    activeChapterId?: string,
    relevantIds: string[] = [],
  ): string {
...
`.trim();
  ...
  static buildStaticArchitectContext(
    project: StoryProject,
  ...
  static buildDynamicArchitectContext(
    project: StoryProject,
    memory: string,
    relevantIds: string[],
    ...
    const inst =
      lang === 'ja'
        ? {
            redacted:
              'ユーザーは現在、設定（聖書）の参照を無効化しています。既存の設定に縛られず、自由な発想で提案を行います。',
        ...
        : {
            redacted:
              'The user has currently disabled Bible reference. Please propose freely without being bound by existing settings.',
  ...
  static buildArchitectSystemInstruction(
    project: StoryProject,
    memory: string,
    relevantIds: string[],
    isContextActive: boolean = true,
    ...
    let systemContent = '';
    if (useCache) {
      systemContent = this.buildDynamicArchitectContext(
        project,
        memory,
        relevantIds,
        isContextActive,
      ...
      const dynamicPart = this.buildDynamicArchitectContext(
        project,
        memory,
        relevantIds,
        isContextActive,

...
import { StoryProject, ChapterLog, GeminiContent, UsageCallback, LogCallback, DetectionResult, ExtractionResult, BibleIssue, NexusBranch, WhisperAdvice, SyncOperation, CreatorContext, VisualContext } from "../../types";
...
export const chatWithArchitect = (
  ...
  defaultService.architectAgent.chat(history, input, project, memory, allowSearch, onUsage, logCb, isContextActive, relevantIds);
...
export const chatWithArchitectStream = (
  ...
  defaultService.architectAgent.chatStream(history, input, project, memory, allowSearch, onUsage, logCb, isContextActive, relevantIds);
...
export const extractSettingsFromChat = (history: GeminiContent[], project: StoryProject, memory: string, detection: DetectionResult, onUsage: UsageCallback, logCb: LogCallback) =>
  defaultService.extractorAgent.extractSyncOps(history, project, memory, detection, (json) => processSyncOperations(json, 'Extractor', project, detection.isHypothetical), onUsage, logCb);
...
export const genesisFill = (context: CreatorContext, currentProfile: any, fieldLabel: string, onUsage: UsageCallback, logCb: LogCallback) =>
  defaultService.creatorAgent.genesisFill(context, currentProfile, fieldLabel, onUsage, logCb);

export const autoFillItem = (context: CreatorContext, itemType: string, itemName: string, fieldLabel: string, currentItem: any, onUsage: UsageCallback, logCb: LogCallback) =>
  defaultService.creatorAgent.autoFillItem(context, itemType, itemName, fieldLabel, currentItem, onUsage, logCb);

export const brainstormItem = (context: CreatorContext, itemType: string, itemName: string | undefined, currentItem: any, fieldHints: string, onUsage: UsageCallback, logCb: LogCallback) =>
  defaultService.creatorAgent.brainstorm(context, itemType, itemName, currentItem, fieldHints, onUsage, logCb);
...
export const generateDraftStream = (
  ...
  defaultService.writerAgent.streamDraft(chapter, tone, usePro, project, previousContent, targetBeats, logCb, isContextActive, onUsage);
...
export const suggestNextSentence = (content: string, project: StoryProject, activeChapterId: string, onUsage: UsageCallback, logCb: LogCallback, isContextActive: boolean = true) =>
  defaultService.writerAgent.suggest(content, project, activeChapterId, isContextActive, onUsage, logCb);
...
export const scanDraftAppSettings = (draft: string, project: StoryProject, activeChapterId: string, onUsage: UsageCallback, logCb: LogCallback) =>
  defaultService.writerAgent.scanDraft(draft, project, activeChapterId, (json) => processSyncOperations(json, 'DraftScanner', project, false), onUsage, logCb);
...
// --- Visual & Analysis ---
export const generateCharacterPortrait = (character: any, context: VisualContext, onUsage: UsageCallback, logCb: LogCallback) =>
...
export const maintainSummaryBuffer = (project: { bible: any }, onUsage: UsageCallback, logCb: LogCallback) =>
  defaultService.analysisAgent.simulateNexus("要約バッファを維持せよ", project as any, onUsage, logCb).then(res => res.impactOnCanon);

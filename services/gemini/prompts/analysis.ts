...
export const INTEGRITY_SCAN_PROMPT = (lang: AppLanguage, bibleData: string) => {
  return PromptTemplate.from(getTemplate('analysis.integrityScan', lang)).format({
    bible: bibleData,
  });
};

export const DETECTION_PROMPT = (userInput: string, lang: AppLanguage = 'ja') => {
  return PromptTemplate.from(getTemplate('analysis.detectorPrompt', lang)).format({
    input: userInput,
  });
};
...
export const INITIAL_CHAPTERS_PROMPT = (bibleJson: string, lang: AppLanguage) => {
  return PromptTemplate.from(getTemplate('analysis.muse.chapters', lang)).format({
    bible: bibleJson,
  });
};

export const INITIAL_FORESHADOWING_PROMPT = (
  bibleJson: string,
  chaptersJson: string,
  lang: AppLanguage,
) => {
...
export const SAFETY_ALTERNATIVES_PROMPT = (
  input: string,
  _category: string,
  lang: AppLanguage = 'ja',
) => {

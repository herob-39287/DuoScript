...
export class CreatorAgent extends BaseAgent {
  ...
  async autoFillItem(context: CreatorContext, itemType: string, itemName: string, fieldLabel: string, currentItem: any, onUsage: UsageCallback, logCallback: LogCallback): Promise<string> {
    ...
    return this.client.request({
      model: AI_MODELS.FAST,
      contents: Prompts.AUTO_FILL_ITEM_PROMPT(itemType, itemName, fieldLabel, itemStr, worldContext, lang),
  ...
  async brainstorm(
    ...
    const prompt = Prompts.BRAINSTORM_PROMPT(itemType, taskDescription, currentJson, worldContext, fieldHints, lang);
    ...
    return this.client.request({
      ...
      config: {
        ...
        responseSchema: { 
          type: Type.ARRAY, 
          items: { type: Type.OBJECT, properties: { name: { type: Type.STRING }, concept_note: { type: Type.STRING } } } 
        },

...
export class ExtractorAgent extends BaseAgent {
  ...
  async extractSyncOps(
    ...
    const relevantIds = await this.librarian.identifyRelevantEntities(
      conversationText,
      project,
      onUsage,
      logCallback,
    ...
    if (domains.length === 0) {
      tasks.push(
        this._runExtraction(
          basePrompt,
          Prompts.SYNC_EXTRACTOR_SOUL(lang),
          'Architect/Extraction:General',
          processOps,
          lang,
          onUsage,
          logCallback,
      ...
      if (domains.includes('ENTITIES'))
        tasks.push(
          this._runExtraction(
            basePrompt,
            Prompts.EXTRACTOR_SOUL_ENTITIES(lang),
            'Architect/Extraction:Entities',
            processOps,
            lang,
            onUsage,
            logCallback,
      ...
      if (domains.includes('FOUNDATION'))
        tasks.push(
          this._runExtraction(
            basePrompt,
            Prompts.EXTRACTOR_SOUL_FOUNDATION(lang),
            'Architect/Extraction:Foundation',
            processOps,
            lang,
            onUsage,
            logCallback,
      ...
      if (domains.includes('FORESHADOWING'))
        tasks.push(
          this._runExtraction(
            basePrompt,
            Prompts.EXTRACTOR_SOUL_FORESHADOWING(lang),
            'Architect/Extraction:Foreshadowing',
            processOps,
            lang,
            onUsage,
            logCallback,
      ...
      if (domains.includes('NARRATIVE'))
        tasks.push(
          this._runExtraction(
            basePrompt,
            Prompts.EXTRACTOR_SOUL_NARRATIVE(lang),
            'Architect/Extraction:Narrative',
            processOps,
            lang,
            onUsage,
            logCallback,
    ...
    results.forEach((res) => {
      res.readyOps.forEach((op) => {
        ...
        if (seenOps.has(uniqueKey)) {
          if ((op.confidence || 0) > (seenOps.get(uniqueKey)!.confidence || 0))
            seenOps.set(uniqueKey, op);

...
export class AnalysisAgent extends BaseAgent {
  ...
  async scanIntegrity(
    ...
    const chapterMap = project.chapters.map((c) => {
      ...
      return {
        ...
        contentSnippet: activeChapterId === c.id && c.content ? c.content.slice(-2000) : undefined,
      };
    });
    ...
    return this.client.request({
      ...
      logCallback: logCallback,
      mapper: (res) => {
        const data = parseWithSchema<IntegrityScanResponse>(
          res.text,
  ...
  async simulateNexus(
    ...
    return this.client.request({
      ...
      logCallback: logCallback,
      mapper: (res) => {
        const sim = parseWithSchema<NexusSimulationResponse>(
          res.text,
          NexusSimulationZodSchema,
          'Nexus',
          Schemas.DEFAULT_RESPONSES.NEXUS,
        );
        return {
  ...
  async generateProject(
    ...
    logCallback(
      'info',
      'Architect',
      'Step 1/3: 創世記（世界設定・用語）を構築中... (Deep Reasoning)',
    );
    ...
    logCallback(
      'info',
      'Architect',
      'Step 3/3: 確定したプロットに伏線とスレッドを仕込んでいます...',
    );

...
import { 
  LawSyncSchema, LocationSyncSchema, OrganizationSyncSchema,
  KeyItemSyncSchema, EntrySyncSchema, ThemeSyncSchema, RaceSyncSchema,
  BestiarySyncSchema, AbilitySyncSchema, TimelineSyncSchema,
...
export class CollectionStrategy implements SyncStrategy {
  private getMapping(path: string) {
    const modelMap: Record<string, { schema: z.ZodType<any>, naming: string, content: string }> = {
      ...
      'nexusBranches': { schema: NexusBranchSyncSchema, naming: 'hypothesis', content: 'impactOnCanon' }
    };
  ...
  apply(ctx: SyncContext, op: SyncOperation) {
    ...
    if (idx === -1 && op.op !== 'delete') {
      ...
      if (!newItem[namingField]) {
        newItem[namingField] = op.targetName || incomingValue.name || incomingValue.title || incomingValue.event || incomingValue.concept || "無題";
      }
    ...
    if (isBiblePath) {
      ...
      return { nextBible, nextChapters: collection as unknown as ChapterLog[], targetName: String(targetName), oldValue: oldVal, newValue: newVal };
    }
  }
  ...
  revert(ctx: SyncContext, history: HistoryEntry) {
    ...
    if (history.opType === 'delete') {
      ...
      const idx = collection.findIndex((i) => (i.id === history.targetId) || i[namingField] === history.targetName);

...
import { findItemIdx, validateSyncOperation } from './utils';

// Helper to find item index by path
const findItemIdxByPath = (
  bible: WorldBible,
  chapters: ChapterLog[],
  path: string,
  id: string,
...
export const calculateSyncResult = (
  ...
  try {
    const { nextBible, nextChapters, targetName, oldValue, newValue } = strategy.apply(
      { bible, chapters, history },
      op,
    );
    nextBible.version += 1;
    ...
    try {
      ...
      if (op.op === 'update' || op.op === 'delete') {
        if (op.targetId) {
          ...
          if (idx === -1) {
            throw new Error(
              `Integrity Error: Target ID ${op.targetId} not found in ${op.path}. The item may have been deleted.`,
            );
          }
        }
      }

      const { nextBible, nextChapters, historyEntry } = calculateSyncResult(
        currentBible,
        currentChapters,
        op,
        currentHistory,
...
export const calculateRevertResult = (
  bible: WorldBible,
  chapters: ChapterLog[],
  history: HistoryEntry,
): { nextBible: WorldBible; nextChapters: ChapterLog[] } => {
...
export const getCurrentValueForDiff = (
  bible: WorldBible,
  chapters: ChapterLog[],
  path: string,
  targetName?: string,
  ...
  if (Array.isArray(collection)) {
    ...
    if (field) {
      if (path === 'characters') {
        ...
        if (
          ['location', 'health', 'currentGoal', 'internalState', 'socialStanding'].includes(field)
        ) {

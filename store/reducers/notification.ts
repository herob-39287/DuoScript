  ...
  return produce(state, (draft) => {
    switch (action.type) {
      case 'ADD_LOG': {
        ...
        const isNotifyTarget = (['error', 'success', 'usage', 'Safety'] as const).some(
          (t) => t === log.type || t === log.source,
        );

        if (isNotifyTarget) {
          const type =
            notifTypeMap[log.type as string] || notifTypeMap[log.source as string] || 'info';

...
export const useArchitectChat = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const historyContent: GeminiContent[] =
        PromptBuilder.buildCompressedHistory(currentSyncState);
      ...
      if (isContextActive) {
        ...
        const contextText =
          historyContent.map((h) => h.parts[0].text).join('\n') + '\n' + userMsg.content;
        try {
      ...
      if (e.message?.includes('SAFETY_BLOCK')) {
        const warningMsg: ChatMessage = {
          ...
          role: 'model',
          content:
            '⚠️ 安全ガイドラインにより、この応答はブロックされました。別の表現で試すか、話題を変更してください。',
          timestamp: Date.now(),
          kind: 'system_note',
        };

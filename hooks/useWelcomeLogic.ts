    ...
    try {
      const generated = await generateRandomProject(
        ...
        (type, source, msg, detail) => {
          ...
          if (msg.includes('Step') || msg.includes('構築') || msg.includes('生成') || msg.includes('Generating')) {
             setProgressMsg(msg);
          }
        }
      );
      ...
      if (rawError.includes('403') || rawError.includes('permission') || rawError.includes('PERMISSION_DENIED')) {
        errorMsg = lang === 'ja' 
